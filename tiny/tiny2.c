...
void doit(int fd) 
{
    ...
    if (!Rio_readlineb(&rio, buf, MAXLINE))  //line:netp:doit:readrequest
        return; //요청헤드를 buf에 읽어온다
    printf("%s", buf);
    sscanf(buf, "%s %s %s", method, uri, version); // 요청 헤더에서 메스드, url및 버전을 추출한다
    if (strcasecmp(method, "GET")) {                     //line:netp:doit:beginrequesterr
        clienterror(fd, method, "501", "Not Implemented",
                    "Tiny does not implement this method"); //404오류를 클라이언트에게 반환하고 함수를 종료합니다
        return;
...
int parse_uri(char *uri, char *filename, char *cgiargs) 
{
	...
	strcpy(filename, ".");      // .로 초기화                     //line:netp:parseuri:beginconvert1
	strcat(filename, uri);      // uri를 이어 붙인다.                     //line:netp:parseuri:endconvert1
	if (uri[strlen(uri)-1] == '/')                   //line:netp:parseuri:slashcheck
	    strcat(filename, "home.html");  // 마지막 문자가 '/'면 home.html을 이어 붙인다.             //line:netp:parseuri:appenddefault
	return 1;
    }
    else {  /* Dynamic content */                        //line:netp:parseuri:isdynamic
	ptr = index(uri, '?');   // ?를 찾는다                        //line:netp:parseuri:beginextract
	if (ptr) {// 문자를 찾는다면
	    strcpy(cgiargs, ptr+1);// cgiargs에 ptr 다음 문자부터 끝까지의 문자열을 복사한다.
	    *ptr = '\0'; //ptr \0로 설정하여 uri문자열을 종료한다.
	}
	else 
	    strcpy(cgiargs, "");    // 빈문자로 초기화 해버림                     //line:netp:parseuri:endextract
	strcpy(filename, ".");                           //line:netp:parseuri:beginconvert2
	strcat(filename, uri);      // 이어 붙인다.                     //line:netp:parseuri:endconvert2
	return 0;
    }
}
/* $end parse_uri */
